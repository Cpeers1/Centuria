...
public class GameKinoParlor extends AbstractMinigame {
    ...
    private void sendGameCommand(String command, String[] arguments, Player player){
        if(command == "multiplierResults"){
            ...
            LevelEventBus.dispatch(new LevelEvent("levelevents.minigames.kinoparlor",
            new String[] { "won a game" }, player));
        }
...
/*
    @MinigameMessage("multiplier")
	public void multiplier(Player player, XtReader rd){
        multiplier = rd.readInt();
        sendGameCommand("multiplierResults", String.valueOf(multiplier), player);
    }
*/
    ...
    private class QueensDuel{
        ...
        static QueensDuelDiceFace[] diceFaces = 
            {
                QueensDuelDiceFace.Sword,
                QueensDuelDiceFace.Mask,
                QueensDuelDiceFace.Helmet,
                QueensDuelDiceFace.Mask,
                QueensDuelDiceFace.Crown,
                QueensDuelDiceFace.Sword
            };

        static final float[] binomcdf = {0.0156f, 0.1094f, 0.3438f, 0.6562f, 0.8906f, 0.9844f, 1f}; // binomial cumulative distribution function, 6 trials, probability 0.5
        ...
        public QueensDuel(){
            ...
            kinosRoll = new int[6];
            kinosLanes = new int[][]{{9, 9, 9, 9},
                                    {9, 9, 9, 9},
                                    {9, 9, 9, 9}}; // each element is a dice from the roll
            turn = 0;
            tieCount = 0;
            winCount = 0;
            loseCount = 0;
        ...
        private boolean canAddToLane(List<QueensDuelDiceFace> lane, QueensDuelDiceFace newFace){
            if(newFace != QueensDuelDiceFace.Sword){
                return !(lane.contains(QueensDuelDiceFace.Crown) ||
                    lane.contains(QueensDuelDiceFace.Helmet) || 
        ...
        private List<List<QueensDuelDiceFace>> bestLaneConfiguration(int[] rolls){
            ...
            while(true){
                ...
                if(faceIndex != -1 && laneIndex != -1){   
                    Centuria.logger.debug(Integer.toString(laneIndex) + " " + rolledFaces.get(faceIndex).toString() + " " + Float.toString(highestBinomcdfDiff));
        ...
        private void command(Player player, XtReader rd, String command, String[] gameCommandParameters){
            switch(command){
                ...
                case "requestCompare":
                    ...
                    loseCount = 0;
                    for(int i = 0; i < 3; i++){
                        if(getLaneScore(stringToIntArray(gameCommandParameters[i]), playersRoll) > getLaneScore(kinosLanes[i], kinosRoll)){
                            winCount++;
                        } else if (getLaneScore(stringToIntArray(gameCommandParameters[i]), playersRoll) < getLaneScore(kinosLanes[i], kinosRoll)){
                            loseCount++;
                        } else {
                            tieCount++;
                        }
                ...
                case "requestTieRoll":
                    ...
                    for(int i = 0; i < tieCount; i++){
                        ...
                        if(diceFaces[playersTieBreakingRolls[i]-1].ordinal() > 
                            diceFaces[kinosTieBreakingRolls[i]-1].ordinal()){

                            winCount++;
                        } else if (diceFaces[playersTieBreakingRolls[i]-1].ordinal() <
                            diceFaces[kinosTieBreakingRolls[i]-1].ordinal()){
    ...
    private class FourCrows{
        ...
        static final float[] binompdf = {0.07776f, 0.2592f, 0.3456f, 0.2304f, 0.0768f, 0f}; // binomial point distribution function, 5 trials, probability 0.4, last value set to 0
        ...
        private List<Integer> getParams(){
            ...
            return IntStream.of(suitOfCards)
                .boxed()
        ...
        private void command(Player player, XtReader rd, String command, String[] gameCommandParameters){
            switch(command){
                ...
                case "playerPlay":
                    ...
                    Centuria.logger.debug(Integer.toString(moths) + " " + Integer.toString(fires) + " " + Integer.toString(guess));
