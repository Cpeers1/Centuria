		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (btnNewButton.getText().equals("Start")) {
					// Start servers
					director = new ConnectiveHTTPServer();
					api = new ConnectiveHTTPServer();
					chat = new BasicProxyServer();
					
					// Director
					try {						
						director.setIp(InetAddress.getLoopbackAddress());
						director.setPort(Integer.parseInt(textField_4.getText()));
						director.registerProcessor(new DirectorProcessor());
						director.start();
					} catch (Exception e2) {
						JOptionPane.showMessageDialog(frmLegacyClientPacket, "Failed to start the director server, please check the configuration.", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}
					
					// API
					try {
						api.setIp(InetAddress.getLoopbackAddress());
						api.setPort(Integer.parseInt(textField_5.getText()));
						api.registerProcessor(new ProxyHttpProcessor(txtHttpsaerialworksddnsnet.getText()));
						api.start();
					} catch (Exception e2) {
						JOptionPane.showMessageDialog(frmLegacyClientPacket, "Failed to start the API server, please check the configuration.", "Error", JOptionPane.ERROR_MESSAGE);
						director.stop();
						return;
					}
					...
					try {
						...
						JOptionPane.showMessageDialog(frmLegacyClientPacket, "Failed to start the chat server, please check the configuration.", "Error", JOptionPane.ERROR_MESSAGE);
						director.stop();
						api.stop();
						return;
					}
					...
					try {
						...
						JOptionPane.showMessageDialog(frmLegacyClientPacket, "Failed to start the game server, please check the configuration.", "Error", JOptionPane.ERROR_MESSAGE);
						director.stop();
						api.stop();
						chat.stop();
						return;
