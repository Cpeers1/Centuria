package org.asf.centuria.entities.components.twiggles;
...
public class TwiggleComponent extends InventoryItemComponent {
	...
	protected void getPropertiesFromJson(JsonObject object) {
		...
		if (object.has(WORK_PARAMS_PROPERTY_NAME)) {
			try {
				this.twiggleWorkParams = (TwiggleWorkParameters) new TwiggleWorkParameters()
						.CreateObjectFromJson(object.get(WORK_PARAMS_PROPERTY_NAME).getAsJsonObject());
			} catch (Exception e) {
