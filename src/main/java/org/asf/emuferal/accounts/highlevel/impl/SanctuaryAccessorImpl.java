...
public class SanctuaryAccessorImpl extends SanctuaryAccessor {
	...
	public void saveSanctuaryLookToSlot(String activeSancLookId, String slotId, String saveName) {
		...
		if(oldSancSlot == null) return; //cannot find
		...
		if(newSancSlot == null) return; //cannot find
		...
		var oldInfoComponent = oldSancSlot.getAsJsonObject()
				.get("components").getAsJsonObject()
				.get("SanctuaryLook").getAsJsonObject()
				.get("info").getAsJsonObject();
		
		var newInfoComponent = newSancSlot.getAsJsonObject()
				.get("components").getAsJsonObject()
				.get("SanctuaryLook").getAsJsonObject()
				.get("info").getAsJsonObject();
	...
	private void copyClassInv(String oldClassInvId, String newClassInvId, long timeStamp) {
		...
		if(oldClassInv == null) return; //cannot find
		...
		if(newClassInv == null) return; //cannot find
		
		//copy old island inv to new house inv
		
		var oldComponentLevel = 
				oldClassInv.getAsJsonObject()
				.get("components").getAsJsonObject();
		
		var classLevel = 
				oldComponentLevel.get("SanctuaryClass");
		
		//deep copy it
		var classCopy = classLevel.deepCopy();
		
		//replace island copy
		var newComponentLevel =
				newClassInv.getAsJsonObject()
				.get("components").getAsJsonObject();
	...
	private void copyIslandInv(String oldIslandInvId, String newIslandInvId, long timeStamp) {
		...
		if(oldIslandInv == null) return; //cannot find
		...
		if(newIslandInv == null) return; //cannot find
		
		//copy old island inv to new house inv
		
		var oldComponentLevel = 
				oldIslandInv.getAsJsonObject()
				.get("components").getAsJsonObject();
		
		var islandLevel = 
				oldComponentLevel.get("Island");
	...
	private void copyHouseInv(String oldHouseInvId, String newHouseInvId, long timeStamp) {
		...
		if(oldHouseInv == null) return; //cannot find
		...
		if(newHouseInv == null) return; //cannot find
		
		//copy old house inv to new house inv
		
		var oldComponentLevel = 
				oldHouseInv.getAsJsonObject()
				.get("components").getAsJsonObject();
		
		var houseLevel = 
				oldComponentLevel.get("House");
	...
	private void copySancLookInv(JsonArray looks, JsonElement newSancSlot, JsonElement oldSancSlot, String saveName, long timeStamp) {
		...
		var oldInfoComponent = oldComponentLevel
				.get("SanctuaryLook").getAsJsonObject()
				.get("info").getAsJsonObject();
		
		var newComponentLevel = 
				newSancSlot.getAsJsonObject()
				.get("components").getAsJsonObject();
		
		var newInfoComponent = newComponentLevel
				.get("SanctuaryLook").getAsJsonObject()
				.get("info").getAsJsonObject();
		
		var oldPlacementsComponent = oldInfoComponent
